...
void TodosArchivos(){
    ...
    for (int i=0; i<nficheros; i++){
        ...
        alg_Greedy(n, mat1, mat2, sol, &costo);
			//	alg_Busqueda_Local(n, mat1, mat2, sol, &costo, true);
        tiempo = elapsed_time();
        finales[i][0]=costo;
        tiempos[i][0]=tiempo;
        ...
        desviaciones[i][0] = desviaciones[i][0]*10;
			  ...
			  //Aplico Busqueda Multiarranque Basica
        start_timers();
        alg_Busqueda_Multiarranque_Basica(n, mat1, mat2, sol, &costo);
        tiempo = elapsed_time();
        finales[i][1]=costo;
        tiempos[i][1]=tiempo;
        ...
        desviaciones[i][1] = desviaciones[i][1]*10;
			  ...
			  //Aplico Graps
			  start_timers();
			  alg_Graps(n, mat1, mat2, sol, &costo);
			  tiempo = elapsed_time();
			  finales[i][2]=costo;
			  tiempos[i][2]=tiempo;
			  ...
			  //Aplico Busqueda Local Reiterada
			  start_timers();
			  alg_Busqueda_Local_Reiterada(n, mat1, mat2, sol, &costo);
			  tiempo = elapsed_time();
			  finales[i][3]=costo;
			  tiempos[i][3]=tiempo;
...
int main(int argc, char * argv[])
{
    ...
    start_timers();
		alg_Busqueda_Multiarranque_Basica(n, mat1, mat2, sol, &costo);
    tiempo = elapsed_time();
		printf("Busqueda Multiarranque Basica de %d datos: Coste-> %d. Tiempo-> %fs.\n", n, costo, tiempo);
		printf("Solucion encontrada: ");
		for (int i=0; i<n; i++){
			printf("%d ", sol[i]);
		}
		...
		//Aplico Busqueda Graps
		start_timers();
		alg_Graps(n, mat1, mat2, sol, &costo);
		tiempo = elapsed_time();
		printf("Busqueda Graps de %d datos: Coste-> %d. Tiempo-> %fs.\n", n, costo, tiempo);
		printf("Solucion encontrada: ");
		for (int i=0; i<n; i++){
			printf("%d ", sol[i]);
		}
		...
		alg_Busqueda_Local_Reiterada(n, mat1, mat2, sol, &costo);
		tiempo = elapsed_time();
		printf("Busqueda Local Reiterada de %d datos: Coste-> %d. Tiempo-> %fs.\n", n, costo, tiempo);
		printf("Solucion encontrada: ");
		for (int i=0; i<n; i++){
			printf("%d ", sol[i]);
		}
