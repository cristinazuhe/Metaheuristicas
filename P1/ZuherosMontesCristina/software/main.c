...
void TodosArchivos(){
    ...
    for (int i=0; i<nficheros; i++){
     ...
     start_timers();
        ...
        start_timers();
        alg_Enfriamiento_Simulado(n, sol, &costo);
        tiempo = elapsed_time();
        finales[i][2]=costo;
        tiempos[i][2]=tiempo;
       ...
       //Aplico Busqueda Tabu
        start_timers();
        alg_Busqueda_Tabu(n, mat1, mat2, sol, &costo);
        tiempo = elapsed_time();
        finales[i][3]=costo;
        tiempos[i][3]=tiempo;
        ...
        desviaciones[i][3] = desviaciones[i][3]*10;
				...
				//Aplico Busqueda Tabu
				start_timers();
				alg_Busqueda_Tabu_Ext(n, mat1, mat2, sol, &costo);
				tiempo = elapsed_time();
				finales[i][4]=costo;
				tiempos[i][4]=tiempo;
...
int main(int argc, char * argv[])
{
    ...
    start_timers();
		alg_Enfriamiento_Simulado(n, sol, &costo);
		tiempo = elapsed_time();
		printf("Enfriamiento Simulado de %d datos: Coste-> %d. Tiempo-> %fs.\n", n, costo, tiempo);
		printf("Solucion encontrada: ");
		for (int i=0; i<n; i++){
			printf("%d ", sol[i]);
		}
		...
		alg_Busqueda_Tabu(n, mat1, mat2, sol, &costo);
		tiempo = elapsed_time();
		printf("Busqueda Tabu de %d datos: Coste-> %d. Tiempo-> %fs.\n", n, costo, tiempo);
		printf("Solucion encontrada: ");
		for (int i=0; i<n; i++){
			printf("%d ", sol[i]);
		}
		printf("\n\n");
                ...
                //Aplico Busqueda Tabu Extendida
                start_timers();
		alg_Busqueda_Tabu_Ext(n, mat1, mat2, sol, &costo);
		tiempo = elapsed_time();
		printf("Busqueda Tabu extendida de %d datos: Coste-> %d. Tiempo-> %fs.\n", n, costo, tiempo);
		printf("Solucion encontrada: ");
		for (int i=0; i<n; i++){
			printf("%d ", sol[i]);
		}
