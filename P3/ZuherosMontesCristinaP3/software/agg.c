...
void alg_AGEstacionarios(int n, int **mat1, int **mat2, int *solucion, int *costo, int tipo_operador){
  ...
  while (n_evaluaciones < 25000){ //BUCLE PRINCIPAL....
      //SELECCION. Creo poblacion de padres:obtengo dos aleatorios y me quedo con el mejor de la poblacion actual
      int poblacionpadres[2][n+1];
      seleccion_padres(poblacionactual, poblacionpadres, 2);

      //CRUCE
      int poblacionintermedia[2][n+1];
      int indice_auxiliar=0;
            if(tipo_operador==0){ //Hago cruce posicion
              int solcomunes[n];
              int solsueltas[n];
              ...
              //Me quedo con los elementos que no coinciden y los coloco en solsueltas, almacenando sus lugares en poslibres
              for (int j=0; j<n; j++){
                  if (poblacionpadres[0][j]==poblacionpadres[1][j]){
                      poblacionintermedia[0][j]=poblacionpadres[0][j];
                      poblacionintermedia[1][j]=poblacionpadres[0][j];
                      solcomunes[j]=poblacionpadres[0][j];
                  }
                  else {
                      solsueltas[numsuletas]=poblacionpadres[0][j];
                      poslibres[numsuletas]=j;
                      numsuletas++;
                  }
              ...
              //Aniado la permutación realizada en orden al primer hijo.
              for (int j=0; j<numsuletas; j++){
                  poblacionintermedia[0][poslibres[j]]=solsueltas[j];
                  solcomunes[poslibres[j]]=solsueltas[j];
              }
              //Obtengo el coste del primer hijo y se lo aniado
              int cos1 = costeSolucion(solcomunes,n);  //Obtengo el coste de la solucion actual
              poblacionintermedia[0][n]=cos1;
              ...
              //Aniado la permutación realizada en orden al segundo hijo.
              for (int j=0; j<numsuletas; j++){
                  poblacionintermedia[1][poslibres[j]]=solsueltas[j];
                  solcomunes[poslibres[j]]=solsueltas[j];
              }

              //Obtengo el coste del primer hijo y se lo aniado
              int cos2 = costeSolucion(solcomunes,n);  //Obtengo el coste de la solucion actual
              poblacionintermedia[1][n]=cos2;
              ...
              indice_auxiliar++;
                  ...
                  for (int j=auxi1;j<=auxi2;j++){
                      poblacionintermedia[0][j]=poblacionpadres[0][j];
                      hijo1[contador_aux]=poblacionpadres[0][j];
                      poblacionintermedia[1][j]=poblacionpadres[1][j];
                      hijo2[contador_aux]=poblacionpadres[1][j];
                      contador_aux++;
                  }
                  ...
                  int cont1=0, cont2=0;
                      for (int j=0;j<n;j++){
                          ...
                          for (int k=0;k<n_aniadidos;k++){
                              if (metidohijo1==0){
                                  if (poblacionpadres[1][j]==hijo1[k])
                                      metidohijo1=1;
                              }
                              if (metidohijo2==0){
                                  if (poblacionpadres[0][j]==hijo2[k])
                                      metidohijo2=1;
                              }
                          }
                          if (metidohijo1==0){
                             parte2hijo1[cont1]=poblacionpadres[1][j];
                             cont1++;
                          }
                          if (metidohijo2==0){
                             parte2hijo2[cont2]=poblacionpadres[0][j];
                              cont2++;
                          }
                    }
                  ...
                  int nuevo_indice=(auxi2+1)%n;
                  for (int j=0; j<n_introducir; j++){
                          poblacionintermedia[0][nuevo_indice]=parte2hijo1[j];
                          poblacionintermedia[1][nuevo_indice]=parte2hijo2[j];
                          nuevo_indice = (nuevo_indice+1)%n;
                  }

                  int hij1[n],hij2[n];
                  for (int j=0; j<n; j++){
                         hij1[j]=poblacionintermedia[0][j];
                         hij2[j]=poblacionintermedia[1][j];
                  }

                  //Almaceno el coste de los nuevos hijos
                  poblacionintermedia[0][n] = costeSolucion(hij1,n);
                  poblacionintermedia[1][n] = costeSolucion(hij2,n);
           ...
           //MUTACION
           int nummutaciones = 0.001 * (M*n);
           mutar_poblacion(poblacionintermedia, nummutaciones, 2);
           n_evaluaciones = n_evaluaciones+nummutaciones;


      //REEMPLAZAMIENTO
      //Busco los dos peores y los comparo con los hijos para sustituir
      int poblacionnueva[4][n+1];
      int meto0=0;
      if (poblacionintermedia[0][n]<poblacionactual[M-2][n] || poblacionintermedia[0][n]<poblacionactual[M-1][n]) meto0=1;
      if (meto0==1){
          if (poblacionintermedia[0][n] < poblacionactual[M-2][n]){
              for (int i=0; i<n+1; i++){
                  poblacionactual[M-1][i] = poblacionactual[M-2][i];
                  poblacionactual[M-2][i] = poblacionintermedia[0][i];
              }
          }
          else {
              for (int i=0; i<n+1; i++){
                  poblacionactual[M-1][i] = poblacionintermedia[0][i];
              }
          }
      }
      int meto1=0;
      if (poblacionintermedia[1][n]<poblacionactual[M-2][n] || poblacionintermedia[1][n]<poblacionactual[M-1][n]) meto1=1;
      if (meto1==1){
          if (poblacionintermedia[1][n] < poblacionactual[M-2][n]){
              for (int i=0; i<n+1; i++){
                  poblacionactual[M-1][i] = poblacionactual[M-2][i];
                  poblacionactual[M-2][i] = poblacionintermedia[1][i];
              }
          }
          else {
              for (int i=0; i<n+1; i++){
                  poblacionactual[M-1][i] = poblacionintermedia[1][i];
              }
          }
      }


      //Ordeno la poblacion actual
      ordenar_Poblacion(poblacionactual);
