...
void TodosArchivos(){
    ...
    for (int i=0; i<nficheros; i++){
        ...
        start_timers();
        alg_AGGeneracionales(n, mat1, mat2, sol, &costo,0);
        tiempo = elapsed_time();
        finales[i][1]=costo;
        tiempos[i][1]=tiempo;
        ...
        desviaciones[i][1] = desviaciones[i][1]*10;
				...
				//Aplico AGG con OX
				start_timers();
				alg_AGGeneracionales(n, mat1, mat2, sol, &costo,1);
				tiempo = elapsed_time();
				finales[i][2]=costo;
				tiempos[i][2]=tiempo;
				...
				//Aplico AGE posicional
				start_timers();
				alg_AGEstacionarios(n, mat1, mat2, sol, &costo,0);
				tiempo = elapsed_time();
				finales[i][3]=costo;
				tiempos[i][3]=tiempo;
				...
				//Aplico AGE OX
				start_timers();
				alg_AGEstacionarios(n, mat1, mat2, sol, &costo,1);
				tiempo = elapsed_time();
				finales[i][4]=costo;
				tiempos[i][4]=tiempo;
				desviaciones[i][4]=(finales[i][4] - resultadosoptimos[i])*10;
				desviaciones[i][4] = desviaciones[i][4]/resultadosoptimos[i];
				desviaciones[i][4] = desviaciones[i][4]*10;
   ...
   int cont1=0,cont2=0,cont3=0,cont4=0, cont5=0;
    for (int i=0; i<nficheros; i++){
        ...
        if (finales[i][mejor]>=finales[i][1])
		    ...
		    if (finales[i][mejor]>=finales[i][3])
		        mejor=3;
		    if (finales[i][mejor]>=finales[i][4])
		        mejor=4;
        if (mejor == 0) cont1++;
        if (mejor == 1) cont2++;
        if (mejor == 2) cont3++;
        ...
        printf("\tAGG (Posicional): %d en %f\n", finales[i][1],tiempos[i][1]);
        printf("\tAGG (OX): %d en %f\n", finales[i][2],tiempos[i][2]);
        printf("\tAGE (Posicional): %d en %f\n", finales[i][3],tiempos[i][3]);
        printf("\tAGE (OX): %d en %f\n", finales[i][4],tiempos[i][4]);
    }
	...
	desviacion_total[4] += desviaciones[j][4];
   }
   ...
   desviacion_total[4] = desviacion_total[4]/nficheros;

   printf("\nDesviaciones: \n Greedy: %f\n AGG(pos): %f\n AGG(OX): %f\n AGE(pos): %f\n AGE(OX): %f\n",
		desviacion_total[0], desviacion_total[1], desviacion_total[2], desviacion_total[3], desviacion_total[4]);
}
...
int main(int argc, char * argv[])
{
    ...
    start_timers();
		...
		//Aplico AGG Operador Posicional
    start_timers();
		alg_AGGeneracionales(n, mat1, mat2, sol, &costo, 0); //0 indica tipo de operador posicional
    tiempo = elapsed_time();
		printf("Algoritmos Geneticos Generacionales (posicional) de %d datos: Coste-> %d. Tiempo-> %fs.\n", n, costo, tiempo);
		printf("Solucion encontrada: ");
		for (int i=0; i<n; i++){
			printf("%d ", sol[i]);
		}
		...
		//Aplico AGG Operador OX
		start_timers();
		alg_AGGeneracionales(n, mat1, mat2, sol, &costo, 1); //1 indica tipo de operador OX
		tiempo = elapsed_time();
		printf("Algoritmos Geneticos Generacionales (OX) de %d datos: Coste-> %d. Tiempo-> %fs.\n", n, costo, tiempo);
		printf("Solucion encontrada: ");
		for (int i=0; i<n; i++){
			printf("%d ", sol[i]);
		}
		...
		//Aplico AGE Operador Posicional
		start_timers();
		alg_AGEstacionarios(n, mat1, mat2, sol, &costo, 0); //0 indica tipo de operador posicional
		tiempo = elapsed_time();
		printf("Algoritmos Geneticos Estacionarios (posicional) de %d datos: Coste-> %d. Tiempo-> %fs.\n", n, costo, tiempo);
		printf("Solucion encontrada: ");
		for (int i=0; i<n; i++){
			printf("%d ", sol[i]);
		}
