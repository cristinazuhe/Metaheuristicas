...
void permutar_Solucion(int *solucion, int n){
	int gen1, gen2, aux;
	for (int i =0; i<n; i++){
		gen1 = obtener_aleatorio(0,copia_n-1);
		gen2 = obtener_aleatorio(0,copia_n-1);
		while(gen1==gen2){
			gen2 = obtener_aleatorio(0,copia_n-1);
		}
		aux = solucion[gen1];
...
void alg_Busqueda_Local(int n, int **mat1, int **mat2, int *solucion, int *costo){
	...
	cop_mat2 = mat2;
	copia_n = n;
	Set_random(100);      //Vamos cambiando la semilla y obtenemos distintos aleatorios
	permutar_Solucion(solucion,n); 	//Genero la solución incial
